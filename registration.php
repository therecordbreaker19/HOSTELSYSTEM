...
$ccity=$_POST['city'];
$query="insert into  registration(room_id,seater,feespm,foodstatus,stayfrom,duration,firstName,middleName,lastName,gender,contactno,email,egycontactno,guardianName,guardianRelation,guardianContactno,Address,CIty) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
$stmt = $mysqli->prepare($query);
$rc=$stmt->bind_param('iiiisissssisississ',$room_id,$seater,$feespm,$foodstatus,$stayfrom,$duration,$fname,$mname,$lname,$gender,$contactno,$email,$emcntno,$gurname,$gurrelation,$gurcntno,$caddress,$ccity);
$stmt->execute();
$stmt->close();
...
$stmt1->bind_param('ssssiss',$fname,$mname,$lname,$gender,$contactno,$email,$contactno);
$stmt1->execute();
...
<input type="radio" value="1" name="foodstatus"> With Food(P 2000.00 Per Month Extra)
</div>
</div>	
